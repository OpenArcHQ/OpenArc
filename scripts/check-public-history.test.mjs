...
test("accepts only the non-personal assistant attribution trailer", () => withRepo((directory) => {
  commit(directory, { message: "test commit\n\nCo-Authored-By: Claude Opus 5 <noreply@anthropic.com>" });
  assert.doesNotThrow(runChecker(directory));
}));

test("rejects an assistant trailer with any other address", () => withRepo((directory) => {
  commit(directory, { message: "test commit\n\nCo-Authored-By: Claude Opus 5 <someone@example.test>" });
