...
test("both CI gates run the grant deployment guard alongside the existing guards", () => {
  ...
  const releaseGates = read(FILES.releaseGates);
  assert.match(releaseGates, /run:\s*node --test scripts\/grant-deployment\.test\.mjs/u, "release-gates must run the grant deployment guard");
  assert.match(releaseGates, /run:\s*node --test scripts\/action-deployment\.test\.mjs/u, "release-gates must keep the action guard");
  assert.match(releaseGates, /run:\s*node --test scripts\/api-namespace-deployment\.test\.mjs/u, "release-gates must keep the api-namespace guard");
  assert.match(releaseGates, /run:\s*node --test scripts\/account-deployment\.test\.mjs/u, "release-gates must keep the account guard");
});
