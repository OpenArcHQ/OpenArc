...
describe('durability migration and readiness', () => {
  it('records schema3 and exposes the durable definer helpers as migrator', async () => {
    ...
    expect(helpers.rows.map((row) => row.proname)).toEqual([
      ...
      'claim_outbox_jobs',
      'claim_payment_attempt_observations',
      'commit_agent_create',
      ...
      'record_leased_payment_attempt_observation',
      ...
      'release_payment_attempt_observation_lease',
    ...
    expect(triggers.rows.map((row) => row.tgname)).toEqual([
      ...
      'payment_attempt_observation_leases_mutation',
  ...
  it('forces RLS on all three durable tables', async () => {
    ...
    expect(rls.rows.map((row) => row.relname)).toEqual([
      ...
      'idempotency_records',
      'outbox_events',
      'payment_attempt_observation_leases',
      'payment_attempts',
