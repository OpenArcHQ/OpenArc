import { ARC_ERC8004, ARC_TESTNET, AgentRegistryEvidenceSchema } from "@openarc/shared";
...
function setup() {
  const begin = vi.fn(async (source: "arc_rpc", _route: "agent_registry", _peer: string | undefined,
    signal: AbortSignal) => new SourceLease(source, 10, async () => undefined, signal));
  ...
  const config = loadConfig({ NODE_ENV: "test", APP_ORIGIN: origin, COMMIT_SHA: "test-sha",
    ...
    SOURCE_PROXY_SECRET: proxySecret, METRICS_TOKEN: metricsToken, SOURCE_MAX_SUBCALLS: "10" });
